...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-md rounded-xl border bg-white p-6">
      <h1 class="text-2xl font-bold text-[var(--brand-red)]">Customer Login</h1>
      <form class="mt-4 space-y-3" [formGroup]="form" (ngSubmit)="submit()">
        ...
        <button class="w-full rounded bg-[var(--brand-red)] py-2 text-white" [disabled]="form.invalid">Login</button>
      </form>
      <p class="mt-3 text-sm">
        New user?
        <a routerLink="/auth/register" class="text-[var(--brand-red)]">Create account</a>
      </p>
    </div>
  `,
})
